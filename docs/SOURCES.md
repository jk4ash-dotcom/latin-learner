...
- **Curated overrides (ship-block):** high-frequency Genesis forms and known false friends force curated primaries (`curated:<key>`), including:
  ...
  - **v0.1.8 Wave 2 (prefer N):** `caeli`/`caelum`/`caelo` → heaven(s) (**NEVER** beer); `dies`/`die`/`diem`/`diei`/`diebus`/`dierum` → day (**NEVER** diesis/quarter tone); `lucem` (+ lux forms) → light (**NEVER** grove); `aqua`/`aquae`/`aquam`/`aquas`/… → water(s) (**NEVER** fetch-water V; do **not** fold `aquilonem`); `tenebrae`/`tenebras`/`tenebris` → darkness N (**NEVER** darken V / teneō); `faciem`/`facie`/`facies` → face N (**NEVER** faciō make — `faciam` etc. stay make; later verse-context for Gen.6.14–16 *facies* = you-will-make); `anima`/`animam`/… → soul/living being (**NEVER** mind-only animus; do **not** fold animant*/animal*/animadvert*); `imaginem` → image; `species`/`speciem` → kind; `stellas` → stars.
